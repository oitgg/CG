...
void Portal::setup_offscreen_rendering(GLuint fbo){
    ...
    // cria um objeto renderbuffer pra guardar a informação de profundidade
    GLuint rboId;
    ...
    // conecta o objeto framebuffer (fbo)
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);

    // junta a textura c/ o ponto de ligação com as cores do FBO 
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                           GL_TEXTURE_2D, hole_texture, 0);

    // junta o renderbuffer p/ o ponto de ligação com a profundidade
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                              GL_RENDERBUFFER, rboId);

    // volta pro framebuffer do sistema
