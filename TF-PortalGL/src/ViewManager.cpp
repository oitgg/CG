...
void ViewManager::mouseMotion(int x, int y){
    ...
    //pula a função se não houverem mudanças na posição do mouse
    //deve ser o suficiente pra evitar gargalo/overload no viewer
    if(x==centerX && y==centerY)
        return;

    //aparentemente muitas chamadas dos ponteiros de deformação travam o viewer no novo sistema, glut velho (?)
